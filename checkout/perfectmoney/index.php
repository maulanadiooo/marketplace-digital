...
$data = $model->db_query($db, "*", "cart", "kode_invoice = '".mysqli_real_escape_string($db, $_GET['query_invoice_pm'])."' AND buyer_id = '".$login['id']."' ");
$data_depo = $model->db_query($db, "*", "deposit", "kode_depo = '".mysqli_real_escape_string($db, $_GET['query_invoice_pm'])."' AND user_id = '".$login['id']."' ");
$data_fitur = $model->db_query($db, "*", "fitur_order", "kode_invoice = '".mysqli_real_escape_string($db, $_GET['query_invoice_pm'])."' AND user_id = '".$login['id']."' ");
...
if($data['count'] > 0){
    ...
    $yo = 'diomaulana040296';
    $key =  encrypt($yo);
    $encrypt_id = encrypt($id_invoice);
    $base_url = $config['web']['base_url'];
    $terima_pm1 = $base_url."checkout/perfectmoney/terima1.php";
    $terima_pm = $base_url."checkout/perfectmoney/terima.php?callback=".$key."&action=".$encrypt_id;
                                        ...
                                        <input type="hidden" name="csrf_token" value="<?php echo $config['csrf_token'] ?>">
                                            ...
                                            <div class="form-group mt-3">
                                                <!-- Identify your business so that you can collect the payments. -->
                                                    ...
                                                    <input type="hidden" name="PAYMENT_BATCH_NUM" value="270419">
    ...
    require '../../template/footer.php';
        ...
        <?php
        require '../../template/footer.php';    
    } elseif($data_fitur['count'] > 0) {
        $nilai_dalamDollars = $data_fitur['rows']['amount']/$pm['rows']['rate_dollar'];
        $nama_pm = $pm['rows']['nama_pemilik_bank'];
        $no_pm = $pm['rows']['no_rek'];
        
        $user_depo = $model->db_query($db, "*", "user", "id = '".$data_fitur['rows']['user_id']."'");
        $yo = 'diomaulana040296';
    $key =  encrypt($yo);
    $encrypt_id = encrypt($id_invoice);
    $base_url = $config['web']['base_url'];
                                            ...
                                            <input type="hidden" name="csrf_token" value="<?php echo $config['csrf_token'] ?>">
                                                ...
                                                <div class="form-group mt-3">
                                                    <!-- Identify your business so that you can collect the payments. -->
                                                        ...
                                                        <input type="hidden" name="PAYMENT_AMOUNT" value="<? echo round($nilai_dalamDollars, 2); ?>">
                                                        <input type="hidden" name="PAYMENT_UNITS" value="USD">
                                                        <input type="hidden" name="SUGGESTED_MEMO" value="Pembelian Fitur  <? echo $data_fitur['rows']['order_fitur'] ?>">
                                                        <input type="hidden" name="PAYMENT_ID" value="<? echo $id_invoice; ?>">
                                                        <input type="hidden" name="STATUS_URL" 
                                                            value="<? echo $terima_pm1 ?>">
                                                        <input type="hidden" name="PAYMENT_URL" 
