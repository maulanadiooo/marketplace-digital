...
if(!isset($_POST)){
    ...
    if (check_input($_POST, $data) == false) {
		...
		$invoice= $db->real_escape_string(trim(htmlspecialchars(htmlentities($_POST['invoice']))));
		$redirect_modal = $db->real_escape_string(trim(htmlspecialchars(htmlentities($_POST['redirect_modal']))));
		...
		if($website['rows']['secret_key'] != null){
    		if ($response->success == false) {
    		    ...
    		    exit(header("Location: ".$config['web']['base_url'].$redirect_modal));
    		} 
		}
		    ...
		    if ($check_user['count'] == 1) {
		        if ($check_user['rows']['status'] == 'Not Verified'){
				    ...
				    if (password_verify($input_post['password'], $check_user['rows']['password']) == true) {
    				    ...
    				    $detail_cart= $model->db_query($db, "*", "cart", "kode_invoice = '$invoice' "); 
                        ...
                        $update_cart = $model->db_update($db, "cart", array('buyer_id' => $check_user['rows']['id']), "kode_invoice = '$invoice'");
    				    $update_order = $model->db_update($db, "orders", array('buyer_id' => $check_user['rows']['id']), "kode_unik = '".$detail_cart['rows']['kode_unik']."'");
    				    if($update_cart == true && $update_order == true){
    				       $_SESSION['result'] = array('alert' => 'success', 'title' => 'Berhasil masuk!', 'msg' => '');   
                            exit(header("Location: ".$config['web']['base_url'].$redirect_modal));  
    				    } else {
    				        $_SESSION['result'] = array('alert' => 'Gagal!', 'title' => 'Gagal!', 'msg' => 'Belanjaan Kamu Tidak Berhasil Tersimpan, SIlahkan Order Ulang');   
                            exit(header("Location: ".$config['web']['base_url'].$redirect_modal)); 
    				    }
