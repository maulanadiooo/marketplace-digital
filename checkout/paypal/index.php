...
$data = $model->db_query($db, "*", "cart", "kode_invoice = '".mysqli_real_escape_string($db, $_GET['query_invoice_paypal'])."' AND buyer_id = '".$login['id']."' ");
$data_depo = $model->db_query($db, "*", "deposit", "kode_depo = '".mysqli_real_escape_string($db, $_GET['query_invoice_paypal'])."' AND user_id = '".$login['id']."' ");
$data_fitur = $model->db_query($db, "*", "fitur_order", "kode_invoice = '".mysqli_real_escape_string($db, $_GET['query_invoice_paypal'])."' AND user_id = '".$login['id']."' ");
...
if($data['count'] > 0){
    ...
    require '../../template/footer.php';
        ...
        $nilai_dalamDollar = $data_fitur['rows']['amount']/$pm['rows']['rate_dollar'];
                                        ...
                                        <input type="hidden" name="csrf_token" value="<?php echo $config['csrf_token'] ?>">
                                            <div class="form-group mt-3">
                                                <!-- Identify your business so that you can collect the payments. -->
                                                    ...
                                                    <!-- Specify details about the item that buyers will purchase. -->
                                                    <input type="hidden" name="item_name" value="Pembelian Fitur  <? echo $data_fitur['rows']['order_fitur'] ?>">
