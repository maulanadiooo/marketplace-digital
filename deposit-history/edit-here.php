...
$orders_infoa = mysqli_query($db, "SELECT * FROM deposit WHERE user_id = '".$login['id']."'  ORDER BY created_at DESC ");
...
$orders_infot = mysqli_query($db, "SELECT * FROM deposit WHERE user_id = '".$login['id']."' ORDER BY created_at DESC LIMIT $awalData,$dataPerHalaman");
                        ...
                        <div class="statement_table table-responsive">
                            <table class="table">
                                ...
                                while ($orders_info = mysqli_fetch_assoc($orders_infot)){
                                    ...
                                    $bank = $model->db_query($db, "*", "bank_information", "id = '".$orders_info['id_bank']."'");
                                    if($orders_info['status'] == 'success'){
                                        $status ='Sukses';
                                    } elseif($orders_info['status'] == 'error'){
                                        $status ='Gagal';
                                    } else {
                                        $status = 'Pending';
                                    }
                                ...
                                <tbody>
                                    <tr>
                                        <td><?=$orders_info['kode_depo']?></td>
                                        <td><?=$bank['rows']['bank']?></td>
                                        <td>Rp <?= number_format($orders_info['amount'],0,',','.') ?></td>
                                        <td><?=$status?></td>
                                        <td><?= format_date(substr($orders_info['created_at'], 0, -9))?>
                                        <?
                                        if($orders_info['status'] == 'pending'){
                                        ?>
                                        <td><a href="<?=$config['web']['base_url']?>invoice_depo/<?=$orders_info['kode_depo']?>"><button class="btn btn-primary" aria-hidden="true" title="Detail">Detail</button></a> </td>
                                        <?
                                        }
                                        ?>
                                    </tr>
                            ...
                            ?>
                            <li class="page-item">
                                <a class="page-link" href="<?= $config['web']['base_url']; ?>deposit-history/<?= $pageActive - 1 ?>" aria-label="Previous">
                            ...
                            ?>   
                                ...
                                <? if ($i == $pageActive){?>
                                <li class="page-item active"><a class="page-link" href="<?= $config['web']['base_url']; ?>deposit-history/<?= $i; ?>"><?= $i ?></a></li>
                                <? } else {?>
                                <li class="page-item"><a class="page-link" href="<?= $config['web']['base_url']; ?>deposit-history/<?= $i; ?>"><?= $i ?></a></li>
                                <? } ?>
                            <?    
                            }
                            ...
                            ?>
                            <li class="page-item">
                                <a class="page-link" href="<?= $config['web']['base_url']; ?>deposit-history/<?= $pageActive + 1 ?>" aria-label="Next">
