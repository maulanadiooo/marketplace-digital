...
if(!isset($_POST)){
    ...
    if (check_input($_POST, $data) == false) {
		...
		if(check_empty($validation) == true){
		    ...
		    $update = $model->db_insert($db, "categories", array('category' => $validation['category'], 'ket' => $validation['ket_category'], 'url' => $url_nama_produk));
		    if($update == true){
		        $_SESSION['result'] = array('alert' => 'success', 'title' => 'Berhasil!', 'msg' => 'Kategori Berhasil Ditambahkan.');
		        exit(header("Location: ".$config['web']['base_url']."administrator/category/"));
		    } else {
		        echo "Gagal Update";
		    }
