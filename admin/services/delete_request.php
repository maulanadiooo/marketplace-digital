...
if(!isset($_POST)){
    exit(header("Location: ".$config['web']['base_url']."administrator/request/"));
} else {
    $data = array('delid');
    if (check_input($_POST, $data) == false) {
		...
		if(check_empty($validation) == true){
		    ...
		    $update = $model->db_delete($db, "permintaan_pembeli", "id = '".$validation['delid']."'");
		    if($update == true){
		        $_SESSION['result'] = array('alert' => 'success', 'title' => 'Berhasil!', 'msg' => 'Request Berhasil Dihapus.');
		        exit(header("Location: ".$config['web']['base_url']."administrator/request/"));
		    } else {
		        echo "Gagal Update";
		    }
